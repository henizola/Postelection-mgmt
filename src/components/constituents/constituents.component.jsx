    ...
    <RegionContext.Consumer>
      {(context) => (
        <table>
          ...
          <tbody>
            {context.region.constitiuents.map((element, index) => (
              <tr key={index}>
                <th>{element.name}</th>
                <th style={{ paddingLeft: "55px" }}>
                  {element.votersRegisterd}
                </th>
                <th>
                  {element.balotsIssued[0]}-{element.balotsIssued[1]}
                </th>
                <th style={{ paddingLeft: "35px" }}>
                  {element.votes === 0 ? (
                ...
                <th style={{ paddingLeft: "55px" }}>
                  {element.ballotsUnused === 0 ? (
                    <span>not submited</span>
                  ) : (
                    element.ballotsUnused
                  )}
                </th>
                <th style={{ paddingLeft: "55px" }}>
                  {element.ballotsWasted === 0 ? (
                    <span>not submited</span>
                  ) : (
                    element.ballotsWasted
                  )}
                </th>
                <th style={{ paddingLeft: "55px" }}>
                  {element.votes === 0 ? (
